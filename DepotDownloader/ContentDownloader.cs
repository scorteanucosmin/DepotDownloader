...
static class ContentDownloader
{
    ...
    public static bool InitializeSteam3(string username, string password)
    {
        ...
        steam3 = new Steam3Session(new SteamUser.LogOnDetails
            {
                Username = username,
    ...
    {
        PublishedFileDetails details = await steam3.GetPublishedFileDetails(appId, publishedFileId);
    ...
    public static async Task DownloadAppAsync(uint appId, List<(uint depotId, ulong manifestId)> depotManifestIds, string branch, string os, string arch, string language, bool lv, bool isUgc)
    {
        ...
        {
            DepotDownloadInfo info = await GetDepotInfo(depotId, appId, manifestId, branch);
            if (info != null)
            {
                infos.Add(info);
            }
    ...
    private static async Task DownloadSteam3Async(List<DepotDownloadInfo> depots)
    {
        ...
        {
            DepotFilesData depotFileData = await ProcessDepotManifestAndFiles(cts, depot, downloadCounter);
            ...
            else
            {
                ...
                do
                {
                    ...
                    try
                    {
                        ...
                        if (steam3.CDNAuthTokens.TryGetValue((depot.DepotId, connection.Host), out TaskCompletionSource<SteamContent.CDNAuthToken> authTokenCallbackPromise))
                        {
                            SteamContent.CDNAuthToken result = await authTokenCallbackPromise.Task;
            ...
            do
            {
                ...
                catch (SteamKitWebRequestException e)
                {
                    ...
                    if (e.StatusCode == HttpStatusCode.Forbidden &&
                        (!steam3.CDNAuthTokens.TryGetValue((depot.DepotId, connection.Host), out TaskCompletionSource<SteamContent.CDNAuthToken> authTokenCallbackPromise) || !authTokenCallbackPromise.Task.IsCompleted))
                    {
                        await steam3.RequestCDNAuthToken(depot.AppId, depot.DepotId, connection);
